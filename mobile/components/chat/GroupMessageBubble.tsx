import { View, Text, StyleSheet, Image, useWindowDimensions, TouchableWithoutFeedback, Linking, TouchableOpacity, Modal } from 'react-native';
...
interface GroupMessageBubbleProps {
  ...
  showAvatar?: boolean; // Show avatar for first message in a sequence
}

function GroupMessageBubbleComponent({ message, onEdit, onDelete, onLike, showAvatar = true }: GroupMessageBubbleProps) {
  // If message is deleted, render nothing (same as MessageBubble)
  ...
  // Get sender info
  const senderName = message.sender?.fullNames || 'Unknown';
  const senderPhoto = message.sender?.photo || 'https://res.cloudinary.com/dpwbmqutn/image/upload/v1762488954/chw/91a10fa7-f5e9-4a0e-ae5b-5d64902548e7.png';
  ...
  return (
    <View
      ...
      {/* 📸 WhatsApp-style: Avatar on left for received messages, positioned at bottom */}
      {!isSent && showAvatar && (
        <Image
          source={{ uri: senderPhoto }}
          style={[styles.avatar, { width: responsiveStyles.avatarSize, height: responsiveStyles.avatarSize, borderRadius: responsiveStyles.avatarSize / 2 }]}
        />
      )}
      {/* Spacer when avatar is hidden (for message grouping) */}
      {!isSent && !showAvatar && (
        <View style={{ width: responsiveStyles.avatarSize + 8 }} />
      )}
                ...
                return (
                  <View style={[styles.mediaContainer, { height: 165 }]}>
                    <WhatsAppVideoMessage
                      ...
                      bubbleHeight={165}
                      onError={(error) => {
                        console.error('[GroupMessageBubble] Video error:', error);
                        // Do NOT open browser — WhatsAppVideoMessage shows its own
                        // "Tap to retry" error UI when onError fires.
                      }}
                    />
...
export const GroupMessageBubble = memo(GroupMessageBubbleComponent);
...
const styles = StyleSheet.create({
  ...
  // 📸 WhatsApp-style: Avatar positioned at bottom-left
  avatar: {
    marginRight: 8,
    marginBottom: 2,
  },
  ...
  // 👤 Sender name at top of bubble (WhatsApp-style)
  senderName: {
    fontWeight: '600',
    color: '#075E54',
    marginBottom: 2,
  ...
  likeBadgeReceived: {
    left: 48, // Account for avatar width
  },
  likeBadgeText: {
    fontSize: 14,
  },
...
function getResponsiveStyles(width: number) {
  ...
  return {
    bubbleMaxWidth: isSmallScreen ? width * 0.75 : isLargeScreen ? width * 0.55 : width * 0.70,
