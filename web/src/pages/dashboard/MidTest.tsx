...
import { getMidTestById, submitMidTestAttempt } from "@/services/midtest.service";
import { IMidTest, IQuestionnaire } from "@/types";

const MidTest: React.FC = () => {
  const { courseId, midTestId } = useParams<{ courseId: string; midTestId: string }>();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const chapterTitle = searchParams.get("chapter") ?? "Chapter";
  const chapterId = searchParams.get("chapterId") ?? "";
  const revisit = searchParams.get("revisit") === "true";
  const afterSlide = parseInt(searchParams.get("afterSlide") ?? "-1");

  const [midTest, setMidTest] = useState<IMidTest | null>(null);
  ...
  const loadMidTest = useCallback(async () => {
    if (!midTestId) return;
    setIsLoading(true);
    setError(null);
    try {
      const data = await getMidTestById(midTestId);
      if (!data) { setError("Mid-test not found."); return; }
      setMidTest(data);
      setQuestions((data.questionnaires ?? []).slice(0, data.questionToBeAnswered));
    } catch {
      setError("Failed to load chapter test. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, [midTestId]);

  useEffect(() => { loadMidTest(); }, [loadMidTest]);
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await submitMidTestAttempt(
        midTest.id,
        questions.map((q) => ({ questionnaireId: q.id, selectedAnswerIds: Array.from(answers[q.id] ?? []) }))
      );
      navigate(`/learn/${courseId}`, { state: { completedChapterId: chapterId, revisit, afterSlide: afterSlide !== -1 ? afterSlide : undefined } });
    } catch {
  ...
  const marksToPass = midTest?.marksToPass ?? 0;
  ...
  if (error && !isSubmitting) {
    return (
      <div className="flex-1 flex flex-col items-center gap-3 justify-center p-8">
        ...
        <Button size="sm" onClick={loadMidTest}>Retry</Button>
      </div>
    );
  }
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-sm font-semibold text-gray-900">Chapter Test</h1>
              <p className="text-xs text-gray-400 truncate">{chapterTitle}</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-xs font-medium text-[#3363AD] bg-[#3363AD]/10 border border-[#3363AD]/20 px-2 py-1 rounded-full">
                Pass: {marksToPass}%
              </span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-3 flex-1 min-w-0">
                    ...
                    {currentQuestion.allowMultiple && (
                      <span className="inline-block text-xs font-medium text-[#3363AD] bg-[#3363AD]/10 rounded-lg px-2.5 py-1">
